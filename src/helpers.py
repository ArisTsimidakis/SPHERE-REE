...
def check_bounds(filename: str, start_idx: int, end_idx: Optional[int]) -> None:
    """Validate that the provided indices are within bounds."""
    df = pd.read_csv(filename)
    total_queries = len(df)
    
    if start_idx < 0 or start_idx >= total_queries:
        raise ValueError(f"Start index {start_idx} is out of bounds (0-{total_queries-1})")
    
    if end_idx is not None:
        if end_idx <= start_idx or end_idx > total_queries:
            raise ValueError(f"End index {end_idx} must be between {start_idx+1} and {total_queries}")
