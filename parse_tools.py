...
config = None  # Global config will be initialized in main()
...
def process_tool_results(tool_name: str, chart_name: str) -> List[Union[Dict, List]]:
    ...
    try:
        if tool_name == "checkov":
            result_file = f"tools_output/checkov/{chart_name}_results.json"
            logger.debug(f"Processing Checkov file: {result_file}")
            return parse_checkov(chart_name, result_file)
        elif tool_name == "datree":
            result_file = f"tools_output/datree/{chart_name}_results.json"
...
def main():
    ...
    chart_names = set()
    for tool in tools:
        tool_dir = Path(f"tools_output/{tool}")
        if tool_dir.exists():
